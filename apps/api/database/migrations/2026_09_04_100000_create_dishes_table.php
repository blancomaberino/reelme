...
return new class extends Migration
{
    public function up(): void
    {
        ...
        // NO BACKFILL HERE, deliberately — and the reasoning is worth keeping,
        // because the first version of this migration did run one.
        //
        // The problem is real: `PlaceAggregations` and `PlaceSourceResource` read
        // this table from the moment it lands, so an empty table is not a cold
        ...
        // still reports a `dishes_updated_at`. That is the difference from
        // T-031's `reelmap:tags:backfill`, which was purely additive.
        //
        // But the fix belongs in `scripts/deploy.sh`, which now runs
        // `reelmap:dishes:backfill` immediately after `artisan migrate`, still
        // inside maintenance mode. The argument for doing it here was that the
        // deploy "runs migrate and nothing else, so a step someone has to
        // remember happens after the outage" — and that premise was simply
        // false: deploy.sh is a file in this repo, and a line in it is a
        // scripted step, not a remembered one.
        //
        // Three things the in-migration version cost for nothing:
        //   1. Schema history would depend on a mutable application service.
        //      Restore an older production dump and re-migrate, and TODAY's
        //      DishMaterializer runs against YESTERDAY's snapshots.
        //   2. The outage became O(corpus), serial, with no progress output and
        //      no resume — all three of which the command already has.
        //   3. The walk existed twice, in two places that had already drifted on
