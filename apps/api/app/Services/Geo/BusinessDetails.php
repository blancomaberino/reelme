...
final readonly class BusinessDetails
{
    ...
    public static function fromArray(array $data): self
    {
        return new self(
            ...
            // `fromStored()`, not `fromProvider()` and not `salvage()`.
            //
            // This rehydrates a CACHED payload, and the cache holds what
            // `toArray()` wrote — our own NORMALIZED shape, not Google's. So the
            // provider parser reads every entry as malformed and returns null,
            // which silently left the column unwritten for every place (caught in
            // review, and the same defect this task already had once, one layer
            // out). `salvage()` reads the right shape but leniently, and a
            // truncated week still wins BusinessEnricher's first-non-empty merge —
            // a dropped Friday evening reads as "Closed".
            openingHoursPeriods: OpeningSchedule::fromStored($data['opening_hours_periods'] ?? null),
