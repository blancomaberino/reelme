...
return [
    ...
    */
    ...
    |--------------------------------------------------------------------------
    |
    | Boost ships this tool disabled. We enable it because it is the reason we
    | run Boost at all: it evaluates PHP in the real application context, so an
    | agent can inspect dev state (models, relationships, config, queue) without
    | writing a throwaway artisan command.
    |
    | It executes arbitrary PHP against the DEV database and is NOT covered by
    | .claude/hooks/guard-destructive-db.sh, which only inspects Bash commands.
    | Set BOOST_TINKER_TOOL_ENABLED=false in apps/api/.env to turn it back off.
    |
    */

    'tinker_tool_enabled' => env('BOOST_TINKER_TOOL_ENABLED', true),
