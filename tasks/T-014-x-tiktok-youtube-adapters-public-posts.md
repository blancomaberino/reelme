...
- 2026-07-20 (follow-up, same PR #115, commit `12c4777`): did the deferred cleanup — renamed `OEmbedAdapter` → `InstagramAdapter`, narrowed it to instagram-only, and folded it onto `FetchesOEmbed` (removed dead youtube/tiktok PROVIDERS + ~30 duplicated HTTP/guard/scalar lines). `OEmbedAdapterTest` → `InstagramAdapterTest` (instagram cases kept, generic failure-mapping retargeted to instagram URLs, youtube/tiktok cases dropped as redundant). Behavior for the instagram path unchanged. 100% coverage on InstagramAdapter; Pint/PHPStan green; Pest 793 passed. Now every platform (instagram/x/tiktok/youtube) has a dedicated metadata adapter over the shared trait — no generic adapter left.
- 2026-07-20 (launch gate, same PR #115, commit `b4f2562`): shipped **Instagram-only launch posture**. `config/ingestion.php` now defaults `platforms.{x,tiktok,youtube}.enabled` to **false** (instagram unlisted → always on). Two layers read the same switch via `AdapterRegistry::platformEnabled()` (now public): (1) `ShareController::store` REJECTS a share whose canonical URL resolves to a disabled platform with a clear 422 ("Sharing from {TikTok} isn't available yet — only Instagram is supported right now") — via the app's custom error envelope `error.details.url`; (2) the adapter chain skips a disabled platform (existing kill switch). Manual/caption shares + unknown-host URLs are NOT gated (only recognised-but-disabled platforms). Added `Platform::label()`. Enabling a source later = one env flip (`INGESTION_TIKTOK_ENABLED=true`), no deploy. Tests: reject-per-platform + accept-once-enabled in SharesApiTest, registry chain test enables platforms explicitly, Platform label unit test. Pest 798 green.
  - **Mobile follow-up (when the toolchain unblocks):** the share-intent/paste screens should surface this 422 nicely (e.g. "Only Instagram is supported right now") and ideally hide non-IG sources in any picker. API is the enforcement point regardless.
