...
### 2026-07-21 — MERGED to main (PR #131, squash `c9662b5`) — status → done

Owner said "Merge everything." Full `/coderabbit` pass done (4 grounded review agents → fixed a 🔴 JSON-Pointer 422-mapping bug + several 🟡; `/simplify` collapsed the failure taxonomy into one `Record<FailureCode,…>` table + extracted a shared `ShareRow`; `/security-review` clean). Mobile CI green; approval receipt recorded for HEAD. Gates: expo lint + tsc + **jest 283**. Device-verified on the iOS dev client (review form edit → drag pin → Publicar → live pin; status stepper + failure states).

**Dev-env issues hit during on-device review (fixed locally, NOT code):** (1) the Sail `artisan serve` process predated the `CONTRACTS_*` compose env vars, so the contracts JSON-schema path 500'd on the review→publish validation — added the vars to `apps/api/.env` (gitignored); the review→publish path is the first thing to exercise the extraction schema at runtime in this Docker setup. (2) Seeding a review-state share must set `shares.analysis_run_id` (the FK `ExtractionCorrector::original()` reads) via direct assignment — it's not in `$fillable`, so `update()` drops it silently.

**Two real app bugs the on-device review surfaced (both fixed + tested, in this PR):** (a) no in-app entry point once the Compartir tab was removed (T-077) → added Profile → "Mis compartidos" (`app/shares/index.tsx`); (b) `router.back()` on a freshly-opened screen (deep link / push T-027 / post-`router.replace`) threw "GO_BACK not handled" → `safeBack` guard.

**Deep-link / share-intent note (NOT fixed here — follow-up):** `expo-share-intent` captures incoming `reelmap://` scheme URLs as share intents; a push-notification deep link into `reelmap://shares/[id]/status` (T-027) or a `reelmap://list/...` link can be mis-classified and bounce the user to the composer. A guard should make `ShareIntentRedirect` ignore intents whose URL is an internal `reelmap://` app route. Relevant to T-027 and the deep-linkable status/review screens.
