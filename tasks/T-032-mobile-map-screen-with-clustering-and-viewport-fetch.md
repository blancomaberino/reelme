...
- **2026-07-12 — DONE (PR #38, squash `7b89062`).** `app/(main)/map.tsx` — the discovery map tab. react-native-maps (Apple Maps iOS), region in a ref (never React state), fetch on `onRegionChangeComplete` debounced 400ms, `useMapPlaces` with quantized bbox + zoom-band keys, 40% padding, `keepPreviousData`. Server clusters below zoom 15; client supercluster (radius 50 / maxZoom 16) at ≥15; cluster tap animates to expand bounds. `@gorhom/bottom-sheet` preview → `/place/[slug]`. Filter bar (price + popular tags + disabled Following) via a scoped zustand store; truncated → "zoom in" chip. Consumes the place-detail lat/lng push params.
- Pure unit-tested helpers: `src/lib/geo.ts` (region↔bbox↔zoom, quantization, padding, antimeridian clamp) + `src/lib/cluster.ts` (supercluster wrapper).
- New deps `@gorhom/bottom-sheet` + `supercluster`; `GestureHandlerRootView` added to the root layout; `supercluster`/`kdbush` added to jest `transformIgnorePatterns`.
- **Adversarial review fixed 2 HIGH + MED + lows**: (1) marker memoization was defeated every fetch — react-query returns a new pins array each settle, so `onPinPress` changed identity → all markers re-rendered; **fixed by reading the latest data from refs** (synced in an effect, not written during render — React Compiler is strict) with `onPress` depending only on the stable zustand action. (2) `tracksViewChanges={false}` froze the selected-marker highlight — now `tracksViewChanges={selected}`; cluster markers keyed on count for a fresh raster. (3) client-cluster items recompute on the raw viewport each settle, not the quantized key (a sub-cell pan no longer blanks edge pins). Plus lat-only push no longer centers on lng 0, and the client-cluster expansion halves latitudeDelta.
- 74 jest tests (memoization guarded on select AND across a refetch). **Verified live on iPhone 17 Pro via Maestro** `e2e/map-flow.yaml`: register → native Apple Maps over Montevideo with € €€ €€€ €€€€ filter chips, a popular-tag chip, Search button, and API-fetched price-glyph pins.
- **Gotchas**: react-native-maps annotations render on the native MKMapView layer and **don't receive Maestro synthetic coordinate taps** — pin→sheet→detail is covered by the component test + the verified place-detail screen, not the map E2E. The real map has no plain "Map" text (only a composite tab a11y label), so Maestro must key on overlays (Search / €€€). React Compiler (`reactCompiler: true`) forbids writing refs during render and setState-in-effect — use effects to sync "latest value" refs. Added a minimal `app/search.tsx` placeholder that T-034 replaces.
