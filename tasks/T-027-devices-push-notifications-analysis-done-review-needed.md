...
**✅✅ MERGED to main (2026-07-22, squash `7e481ee`, PR #133; owner said "You can merge" after on-device/simulator verification "It works").** All CI green (API + Mobile + CodeRabbit). tasks.json T-027 → done. Feature branch deleted. Simulator verification used a throwaway `dev/push-sim-harness` branch (a `__DEV__` Profile screen scheduling LOCAL notifications with the exact `{type,url,share_id}` payloads → same foreground/tap/cold-start handlers, no real token needed) — never merged, deleted after use. Real token registration + soft permission pre-prompt still need physical hardware (`Device.isDevice`).
