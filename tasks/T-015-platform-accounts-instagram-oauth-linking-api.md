...
- **2026-07-20 — PR #116 OPEN** (`gh pr` on branch `feat/T-015-platform-accounts-instagram-oauth`, HEAD `37e57e1`). `/coderabbit` full pass done + approval receipt written (grounding clean; `/security-review` NO vulns — CSPRNG single-use user-bound state, hardcoded Graph host, encrypted+hidden tokens, owner-scoped policy, fixed-scheme deep link; `/simplify` cleanups applied). Second commit `37e57e1` applied review findings: (a) 🟡 efficiency — `InstagramGraphAdapter::fetchMedia` now reuses the `media_url` persisted by `fetchMetadata` (in `raw`) instead of a second authed Graph `/me/media` call; `DownloadMedia` hydrates `raw` and no longer loads the account; public posts by linked users skip Graph entirely; (b) 🟡 robustness — callback wraps the upsert to convert a concurrent unique-violation race into the `conflict` deep link (not a 500) + bails if the initiating user was deleted mid-flow; `findMedia` paginates `/me/media` by cursor (bounded 5 pages); (c) reuse/altitude — shared `InstagramUrl` helper (shortcode/externalId) across both IG adapters, `requestMedia` reuses `FetchesOEmbed::guard()`, `FetchSourcePost` reads `$e->requiresAuth`, dropped the single-use `LoadsLinkedAccount` trait, `services.instagram.timeout` knob, unified error-envelope helper. Gates: Pint + PHPStan L6 + full Pest **825** green. Awaiting CI + USER merge (agent does not self-merge). ON MERGE: flip tasks.json T-015 → done. NOTE: live IG OAuth needs a Professional/Business IG account + Meta app creds; unset ⇒ linking unavailable, keyless oEmbed still runs.

- **2026-07-20 — MERGED to main** (squash `e82d4fe`, PR #116; user said "merge"). tasks.json T-015 → **done**. Final HEAD `521d086` also added `apps/api/docs/instagram-oauth-linking.md` (go-live runbook) + `.env.example` pointer. All CI green (API pass; Contracts/Mobile skipped). Branch deleted.
