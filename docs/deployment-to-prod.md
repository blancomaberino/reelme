...
| **The scheduler cron, for `reelmap:logs:prune`** | The published 14-day log window stops being enforced. Monolog's `daily` driver prunes only when it ROTATES — on the first write of a new day — so an idle deployment holds a user's `?near=` coordinates indefinitely, and it never touches a bare `laravel.log` left by a `single`-era boot (its glob is `laravel-*.log`, which does not match). The scheduled sweep is what makes the policy sentence a deadline; without cron it never runs. Note it is deliberately NOT `onOneServer()` — logs are per-machine files | `php artisan schedule:list \| grep logs:prune` on each box, and a `ls -la storage/logs` showing files older than `LOG_DAILY_DAYS` |
