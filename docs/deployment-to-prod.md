...
| **logrotate on the web-server access log** | The privacy policy says request data is "discarded after a limited time". `?near=lat,lng` is a QUERY PARAMETER, so every request carrying one writes the user's rounded coordinates into nginx/Caddy's access log — the map, `/offers`, and (since T-158) `GET /places` on every zone, dish or open-now change on the Tonight screen, which is the highest-frequency of the three — a sink this repository does not configure. Laravel's own channel prunes at `LOG_DAILY_DAYS`; the access log does not unless the box says so | Nothing. `LogRetentionTest` is green, `laravel-*.log` prunes on schedule, and `/var/log/nginx/*access.log` holds a year of coordinates |
| **The scheduler cron, for `reelmap:logs:prune`** | The published 14-day log window stops being enforced. Monolog's `daily` driver prunes only when it ROTATES — on the first write of a new day — so an idle deployment holds a user's `?near=` coordinates indefinitely, and it never touches a bare `laravel.log` left by a `single`-era boot (its glob is `laravel-*.log`, which does not match). The sweep runs HOURLY, because a window is only as tight as the pass that enforces it. It carries neither `onOneServer()` nor `withoutOverlapping()` on purpose: logs are per-machine files, and the overlap mutex is fleet-wide | `php artisan schedule:list \| grep logs:prune` on **each** box, and `ls -la storage/logs` showing files older than `LOG_DAILY_DAYS`. A run that cannot delete now logs `logs.prune_failed` and exits non-zero, which `onFailure` turns into `logs.prune_failed_run` |
| **The scheduler cron, for `queue:prune-failed`** | `failed_jobs.exception` holds a full stack trace and `payload` the job's arguments — the same request data the policy's 14 days cover, in a table nothing else prunes and `DELETE /me` never reaches. Without cron the window is true of the files and false of the database | Nothing. `select min(failed_at) from failed_jobs` older than 14 days |
| **`zend.exception_ignore_args=On` in the deployed php.ini** | PHP's default is `Off` on many distros (this dev host included; the Sail image happens to suppress args). With it Off, any exception that escapes as a 500 renders its stack trace **with argument values** into `storage/logs/laravel-*.log` — including whatever was passed to `AgeCheck::enforce()`, the date of birth the privacy policy says is never stored. Refused signups no longer report at all (`bootstrap/app.php`), so this is the residual path: an unexpected 500 raised anywhere below the same call | Nothing. And check the RIGHT SAPI: `php -i` reports the CLI's ini, while the stack traces that would carry a date of birth are written by **PHP-FPM**, which loads its own `php.ini` (plus any `php_admin_value` in the pool config). Use `<?php phpinfo();` through the web server, or `php-fpm -i \| grep exception_ignore_args`, then grep the logs for a `'YYYY-MM-DD'` literal inside a `#0` frame |
