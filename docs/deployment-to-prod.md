...
| **The pre-`daily` `laravel.log`** | Monolog's rotating handler globs `laravel-*.log`, so a file left behind from a `single`-era boot never matches and is never pruned — and the `emergency` channel keeps writing to that same bare path. Any environment that ran before T-156 keeps it forever. (Observed on this dev box: a 40 MB `laravel.log`, plus dated files 49 days old, because `daily` prunes only when it rotates, i.e. on the first write of a new day — an idle deployment holds them past the window) | Nothing. Look for a bare `storage/logs/laravel.log` beside the dated ones |
