...
| **logrotate on the web-server access log** | The privacy policy says request data is "discarded after a limited time". `?near=lat,lng` is a QUERY PARAMETER, so every map request writes the user's rounded coordinates into nginx/Caddy's access log — a sink this repository does not configure. Laravel's own channel prunes at `LOG_DAILY_DAYS`; the access log does not unless the box says so | Nothing. `LogRetentionTest` is green, `laravel-*.log` prunes on schedule, and `/var/log/nginx/*access.log` holds a year of coordinates |
