...
5. **Prove each fix bites.** Watch it RED before the fix makes it green — or,
   for a guard over already-correct code with nothing to revert, mutate the
   PRODUCTION code, watch it fail, and restore with an absolute path (CLAUDE.md
   §5).
