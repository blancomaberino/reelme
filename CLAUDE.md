...
- **Media from photo/carousel posts (T-013):** `dev.sh` drops the **`yt-dlp`** binary into the API container so a no-video post's images (all carousel slides + a video cover frame) are pulled and stored as keyframes the model sees. Public posts work as-is; **private/rate-limited Instagram posts need a cookie file** — export a logged-in `cookies.txt` (Netscape format, e.g. a "Get cookies.txt" browser extension), drop it under `apps/api/` (gitignored), and set `INGESTION_IG_COOKIES_PATH` to its in-container path (the app dir mounts at `/var/www/html`, so `/var/www/html/storage/app/ig-cookies.txt`). Without cookies it falls back to the zero-auth oEmbed hero thumbnail. Prod should bake `yt-dlp` into the image rather than relying on `dev.sh`.
